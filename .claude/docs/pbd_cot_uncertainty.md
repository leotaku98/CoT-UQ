...
### 4.3 gBD — Graph Band Depth (Random-Walk Path Band Depth, Order-Invariant)

**Core idea:** estimate band depth at the **path level** using random walks on the reasoning graph as the reference distribution. For each chain $P_i$, generate pairs of random walks $(P_a, P_b)$ on the graph and measure how much of $P_i$ lies inside their geodesic convex hull. This defines centrality relative to the graph's full topological structure — not just the $N$ specific sampled chains — making it more robust to small ensembles.

#### Reference paths via random walk

Rather than sampling reference paths from the ensemble itself (which with $N=5$ is highly sensitive to individual chains), reference paths $P_a$ and $P_b$ are generated by **uniform random walks** on the undirected reasoning graph:

- Start from a uniformly random vertex in $G$
- At each step, choose uniformly among all neighbours
- Walk length = mean length of the $N$ ensemble chains

Uniform transitions capture structural centrality: a chain $P_i$ is central if it occupies the geometric "middle" of the graph, regardless of how many ensemble chains happened to follow any particular transition.

#### Geodesic convex hull of two paths

$$\text{hull}(P_a, P_b) = \{v \in V : \exists\, u \in P_a,\; w \in P_b \text{ s.t. } d(u,v) + d(v,w) = d(u,w)\}$$

Vertex $v$ is inside the hull if it lies on at least one shortest path between some vertex in $P_a$ and some vertex in $P_b$. All-pairs SPL is precomputed once on the undirected graph, so each hull lookup is an $O(|P_a| \cdot |P_b| \cdot |V|)$ table scan.

#### Path band depth

For each trial, score $P_i$ as the fraction of its vertices inside the hull (soft containment):

$$\text{score}(P_i \mid P_a, P_b) = \frac{|\{v \in P_i : v \in \text{hull}(P_a, P_b)\}|}{|P_i|}$$

Over $T$ trials:

$$\text{gBD}(P_i) = \frac{1}{T} \sum_{t=1}^{T} \text{score}(P_i \mid P_a^{(t)}, P_b^{(t)})$$

#### Confidence score

$$\text{confidence} = \alpha \cdot \overline{\text{gBD}} + (1-\alpha) \cdot \text{majority\_frac}, \quad \alpha = 0.10$$

where $\overline{\text{gBD}} = \frac{1}{N}\sum_i \text{gBD}(P_i)$.

#### Order invariance

All chains are represented as vertex sequences; hull containment is checked per-vertex without reference to ordering. Chains $A\to B\to C\to D$ and $A\to C\to B\to D$ visit the same vertices and receive the same score for any given hull.

#### Implementation sketch

```python
def _random_walk(G_undirected, all_vertices, length, rng):
    start = all_vertices[rng.integers(len(all_vertices))]
    path = [start]
    current = start
...
def _compute_hull(path_a, path_b, spl, all_vertices):
    hull = set()
    for u in set(path_a):
        u_spl = spl.get(u, {})
        for w in set(path_b):
            ...
            for v in all_vertices:
                if v not in hull:
                    d_uv, d_vw = u_spl.get(v), w_spl.get(v)
                    if d_uv is not None and d_vw is not None and d_uv + d_vw == d_uw:
                        hull.add(v)
    return hull

def gbd_scores(chain_vseqs, spl, all_vertices, G_undirected, n_trials=200):
    rng = np.random.default_rng()
    walk_length = max(2, int(round(np.mean([len(s) for s in chain_vseqs if s]))))
    scores = []
    for seq in chain_vseqs:
        trial_scores = []
        for _ in range(n_trials):
            p_a = _random_walk(G_undirected, all_vertices, walk_length, rng)
            p_b = _random_walk(G_undirected, all_vertices, walk_length, rng)
            hull = _compute_hull(p_a, p_b, spl, all_vertices)
            trial_scores.append(sum(v in hull for v in seq) / len(seq))
        scores.append(float(np.mean(trial_scores)))
    return scores
```

See `methods/bd_ensemble/gBD_v3.py` for the full implementation.
...
| | pBD | vBD | gBD |
|---|---|---|---|
| **File** | `methods/bd_ensemble/pBD.py` | `methods/bd_ensemble/vBD.py` | `methods/bd_ensemble/gBD_v3.py` |
| **Alignment** | Sequential DTW (monotonic) | None | None |
| **Band defined by** | Centroid of N−1 chains (per step) | Geodesic hull of 2 vertices | Geodesic hull of 2 random walks |
| **Reference distribution** | Ensemble chains | Ensemble vertices | Uniform random walks on graph |
| **Depth level** | Chain (step-wise cosine sim) | Vertex, averaged over chain | Chain (soft hull containment) |
| **Order-sensitive** | Yes | No | No |
| **Graph required** | No | Yes | Yes |
| **Similarity threshold** | No | Yes (clustering) | Yes (clustering) |
| **Sampling** | Deterministic | Monte Carlo (100 trials) | Monte Carlo (200 trials) |
| **gsm8k AUROC** | 0.698 | 0.682 | TBD |
| **svamp AUROC** | 0.747 | 0.726 | TBD |
| **hotpotQA AUROC** | 0.788 | 0.786 | TBD |
