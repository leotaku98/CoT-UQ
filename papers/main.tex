...
untouched. The reported numbers are currently pending and flagged as
[TODO: run experiment].
...
\begin{table}[t]
  ...
  \begin{tabular}{c cc}
    ...
    \bottomrule
  \end{tabular}
  \caption{Band subset size $J$ (vertices for vBD, walks for gBD-v2) on SVAMP,
  Llama-3.1-8B, AUROC ($\times 100$). Default $J{=}2$ for both.
  [TODO: run experiment].}
...
\begin{table}[t]
  ...
  \begin{tabular}{l c}
    \toprule
    \textbf{Walk length $L$} & \textbf{gBD-v2} \\
    \midrule
    3                  & -- \\
    5                  & -- \\
    7                  & -- \\
    ...
    \bottomrule
  \end{tabular}
  \caption{gBD random-walk length $L$ on SVAMP, Llama-3.1-8B, AUROC ($\times 100$).
  ``adaptive'' scales the length to the graph (default). [TODO: run experiment].}
...
cross-connected. Table~\ref{tab:abla-simthr} sweeps $\theta_c$ for both vBD and
gBD-v2, and Table~\ref{tab:abla-crossthr} sweeps $\theta_e$ for gBD-v2 (vBD uses
no soft edges).
...
\begin{table}[t]
  ...
  \begin{tabular}{c cc}
    ...
    \bottomrule
  \end{tabular}
  \caption{Clustering threshold $\theta_c$ on SVAMP, Llama-3.1-8B, AUROC
  ($\times 100$). [TODO: run experiment].}
  \label{tab:abla-simthr}
\end{table}
...
\begin{table}[t]
  ...
  \begin{tabular}{c c}
    ...
    \bottomrule
  \end{tabular}
  \caption{Semantic-edge threshold $\theta_e$ on SVAMP, Llama-3.1-8B, AUROC
  ($\times 100$). [TODO: run experiment].}
  \label{tab:abla-crossthr}
\end{table}
...
Table~\ref{tab:abla-trials} sweeps $T \in \{50, 100, 200, 400\}$ to verify that
the estimates are converged at the default $T = 200$.
...
\begin{table}[t]
  ...
  \begin{tabular}{c cc}
    ...
    \bottomrule
  \end{tabular}
  \caption{Number of Monte-Carlo trials $T$ on SVAMP, Llama-3.1-8B, AUROC
  ($\times 100$). [TODO: run experiment].}
  \label{tab:abla-trials}
\end{table}
...
are sampled; and completing the Llama-2-13B runs on the remaining datasets and
sampling baselines (the ``--'' entries in Table~\ref{tab:main-results}).
