...
unavailable at inference time. The goal of uncertainty quantification (UQ) is to
design a confidence scorer
\begin{equation}
  \hat{s} : y_i \mapsto \hat{s}(y_i) \in [0, 1],
\end{equation}
that assigns higher scores to correct responses than to incorrect ones, so that
thresholding $\hat{s}(y_i) < \tau$ yields a reliable error detector. Because
$\hat{s}$ depends only on model outputs and never on $y_i^\star$, it serves as a
practical proxy for the inaccessible oracle.
...
We study two reasoning regimes. For \emph{mathematical reasoning} we use
GSM8K~\citep{cobbe2021gsm8k}, SVAMP~\citep{patel2021svamp}, and
ASDiv~\citep{miao2020asdiv}; for \emph{multi-hop question answering} we use
HotpotQA~\citep{yang2018hotpotqa} and 2WikiMultihopQA~\citep{ho2020wikimultihop}.
As backbones we use two open models of differing scale and family,
Llama-3.1-8B~\citep{grattafiori2024llama3} and
Llama-2-13B~\citep{touvron2023llama2}. Full dataset statistics and decoding
hyperparameters are deferred to Section~\ref{sec:exp}.
...
corresponding index in $P^{(r)}$. The path depth of $P^{(m)}$ is the mean cosine
similarity between each of its step embeddings and the centroid of the aligned
steps across the other chains,
\begin{equation}
  s^{(m)} = \frac{1}{n_m} \sum_{k=1}^{n_m}
  \cos\!\left( e\big(s_k^{(m)}\big),\;
  \frac{1}{M-1} \sum_{r \neq m} e\big(s_{\sigma_r(k)}^{(r)}\big) \right),
\end{equation}
and the prompt-level confidence is the mean depth over the ensemble,
$\hat{s} = \frac{1}{M} \sum_{m=1}^{M} s^{(m)}$.

Despite capturing uncertainty in the CoT chain, pBD has its own weakness. In Table~\ref{tab:ensemble-example}, the four chains were produced for a
single HotpotQA question and all reach the correct answer ``We Are the Ocean''
through the same underlying logic---establish each band's member count, compare
the two numbers, and conclude---yet they realize that logic very differently. The
two member-count lookups appear in opposite orders (Chains~A and~C state ``We Are
the Ocean'' first, Chains~B and~D state ``The Dream Academy'' first); the
comparison is phrased in several equivalent ways (``$5 > 3$'', ``3 vs.\ 5'',
``3 is fewer than 5''); and the chains range from three to six steps, some
inserting filler such as ``consider the other band'' that carries no new
information. Conceptually the chains are in near-perfect agreement, but pBD
compares step embeddings position by position, so it matches ``We Are the Ocean
has 5 members'' in one chain against ``The Dream Academy has 3 members'' in
another and registers large spurious diversity, deflating the depth exactly where
confidence should be highest. pBD is therefore \emph{order-sensitive}, even though
agreement among reasoning chains is inherently order-invariant. Computationally,
the differing lengths force pBD to establish a common index set for every pair,
which DTW obtains by solving a dynamic program at a cost of $O(n^2 d)$ per pair
and $O(M^2 n^2 d)$ for the full ensemble. Both problems stem from insisting on a
position-wise, sequential view of the chains.
...
\begin{table}[t]
  ...
  \begin{tabular}{c|llll}
    \toprule
    Step & \textbf{Chain A} & \textbf{Chain B} & \textbf{Chain C} & \textbf{Chain D} \\
    \midrule
    1 & WATO is a 5-piece & TDA: a 3-member group & WATO numbers 5 musicians & TDA: 3 members \\
    2 & TDA has 3 members & WATO has 5 members    & consider the other band  & WATO: 5, more than 3 \\
    3 & $5 > 3$           & 3 vs.\ 5              & TDA is a trio (3)        & answer: WATO \\
    4 & WATO is larger    & 5 is larger           & 5 and 3 side by side     & --- \\
    5 & ---               & WATO has more         & 3 is fewer than 5        & --- \\
    6 & ---               & ---                   & WATO wins on size        & --- \\
    \bottomrule
  \end{tabular}
  \caption{Four Chain-of-Thought chains for the HotpotQA question
  \emph{``Which band has more members, We Are the Ocean or The Dream Academy?''}
  (WATO = We Are the Ocean, TDA = The Dream Academy). All four reach the correct
  answer through the same logic, but differ in the order of the two member-count
  lookups, in the wording of the comparison, and in length (three to six steps).
  The chains agree in meaning, yet pBD's position-wise comparison misaligns
  equivalent steps (e.g.\ ``WATO: 5'' against ``TDA: 3'') and counts them as
  diverse, motivating the order-invariant graph of Section~\ref{sec:method:graph}.}
...
in a chain, the graph maps the reordered lookups, the differently-worded
comparisons, and the varied-length chains of Table~\ref{tab:ensemble-example}
onto one shared structure: equivalent reasoning is no longer counted as diversity
and no pairwise alignment is needed. Starting from the embedded steps of all $M$
chains:
\begin{itemize}
...
graph, and then scores a chain by how central the nodes it visits are. A vertex
is central if it lies ``between'' many other reasoning states---a node that the
bulk of the graph encloses---and peripheral if it sits at the fringe. Because
...
\end{equation}
and the prompt-level confidence is again the ensemble mean,
$\hat{s} = \frac{1}{M} \sum_{m=1}^{M} c\text{BD}(P^{(m)})$.
...
entirely new chains. Figure~\ref{fig:reasoning-graph} shows the graph built from
the four chains of Table~\ref{tab:ensemble-example}, with two sampled random walks
highlighted: walk~A stitches Chain~C's framing step onto Chain~B's TDA-first
ordering, while walk~B takes a shorter route straight from the member counts to
the conclusion. Neither appears in any single sampled chain. gBD draws such walks
as its reference paths, so it measures a chain's depth against the full reasoning
space
...
\begin{figure}[t]
  \centering
  % TODO: replace placeholder with the generated figure:
  %   \includegraphics[width=\linewidth]{resources/reasoning_graph}
  %   (run resources/plot_reasoning_graph.py to produce reasoning_graph.pdf)
  \fbox{\begin{minipage}[c][3.5cm][c]{0.98\linewidth}\centering\itshape
  [Placeholder] reasoning-graph figure --- two sampled walks on $G$ built from the
  four chains of Table~\ref{tab:ensemble-example}.\end{minipage}}
  \caption{Reasoning graph induced by the four chains of
  Table~\ref{tab:ensemble-example} (WATO = We Are the Ocean, TDA = The Dream
  Academy). Gray arrows are reasoning-graph edges aggregated across chains; the
  red and blue paths are two random walks drawn by gBD. Walk~A (red),
  ``consider the other band $\to$ TDA:3 $\to$ WATO:5 $\to$ compare $\to$ WATO
  more,'' recombines Chain~C's framing with Chain~B's ordering; walk~B (blue),
  ``TDA:3 $\to$ compare $\to$ WATO more,'' is a shorter route. Neither occurs in
  any single sampled chain. gBD scores chains against such synthesized walks,
  probing the full reasoning space rather than only the observed chains.}
  \label{fig:reasoning-graph}
\end{figure}
...
\end{equation}
The final confidence blends the mean graph depth with the answer-level majority
fraction $\rho$ (the share of chains agreeing with the modal answer),
\begin{equation}
  \hat{s} = \alpha \, \overline{\text{gBD}} + (1 - \alpha)\, \rho,
  \qquad \alpha = 0.10,
\end{equation}
where $\overline{\text{gBD}} = \frac{1}{M}\sum_m \text{gBD}(P^{(m)})$. We
consider two variants for generating the reference walks:
\begin{itemize}
...
first is \emph{verbalized self-probing}, where the model is asked to state the
probability that its own answer is correct in a separate
context~\citep{xiong2024llms}; in addition to the vanilla form we include the
four reasoning-augmented variants of CoT-UQ~\citep{zhang2025cotuq} that condition
the probe on extracted keywords (\textsc{KeyWords}, \textsc{AllKeyWords}) or
reasoning steps (\textsc{KeyStep}, \textsc{AllStep}). The second family is
...
roughly 13.4, 3.7, and 5.6 points respectively, and over the best verbalized
baseline by more than 20 points on HotpotQA. On Llama-2-13B, where the gains are
even larger, gBD-v2 attains 73.13 on GSM8K and 70.74 on SVAMP, exceeding every
verbalized variant by roughly 20 points. The one column not won by a gBD variant
is 2WikiMultihopQA, where vBD leads at 69.22; we return to this case below.
...
\begin{table*}[t]
  ...
  \begin{tabular}{c l cc ccc}
    ...
    \midrule
    \multirow{12}{*}{\textbf{Llama-3.1-8B}}
      & Self-Probing            & 54.92 & 56.67 & 52.78 & 53.79 & 49.79 \\
      & \quad + KeyWords        & 53.79 & 53.49 & 50.98 & 52.79 & 51.47 \\
      & \quad + AllKeyWords     & 54.03 & 53.72 & 54.58 & 54.35 & 51.70 \\
      & \quad + KeyStep         & 53.45 & 53.62 & 51.89 & 49.99 & 51.77 \\
      & \quad + AllStep         & 54.05 & 52.20 & 50.19 & 51.37 & 51.26 \\
    \cmidrule(lr){2-7}
    ...
    \midrule
    \multirow{12}{*}{\textbf{Llama-2-13B}}
      & Self-Probing            & --    & --    & 50.78 & 50.59 & -- \\
      & \quad + KeyWords        & --    & --    & 48.13 & 48.38 & -- \\
      & \quad + AllKeyWords     & --    & --    & 50.48 & 46.46 & -- \\
      & \quad + KeyStep         & --    & --    & 53.02 & 51.25 & -- \\
      & \quad + AllStep         & --    & --    & 49.72 & 48.77 & -- \\
    \cmidrule(lr){2-7}
      & Non-Contradiction       & --    & --    & --    & --    & -- \\
      & Entailment Prob.        & --    & --    & --    & --    & -- \\
      & Semantic Entropy        & --    & --    & --    & --    & -- \\
    \cmidrule(lr){2-7}
      ...
      & vBD                     & --    & --    & 69.47 & 66.22 & -- \\
      & gBD-v1                  & --    & --    & 72.86 & 70.53 & -- \\
      & gBD-v2                  & --    & --    & \textbf{73.13} & \textbf{70.74} & -- \\
    \bottomrule
  \end{tabular}%
  }
  ...
  Entries marked ``--'' are pending and flagged as [TODO: run experiment]:
  Llama-2-13B has not yet been run on the three sampling baselines, nor on any
  method for HotpotQA, 2WikiMultihopQA, or ASDiv.}
...
\textbf{Verbalized confidence is near-useless on reasoning tasks.} Every
self-probing variant hovers in the 48--57 range across both models, i.e.\ barely
distinguishable from random guessing, and the reasoning-augmented variants of
CoT-UQ do not reliably improve on the vanilla probe under our black-box,
ensemble setting. This confirms the premise of our work: prompting a model to
introspect on its own reasoning does not surface usable uncertainty, because the
signal lives in the \emph{structure} of the reasoning rather than in the model's
verbal report about it.

\textbf{Structure beats answer agreement.} Sampling-based consistency methods are
substantially stronger than verbalized ones, confirming that cross-sample
agreement carries real signal. Yet they are themselves clearly surpassed by the
band-depth family on every dataset. Because both operate on the same ensemble of
sampled outputs, the gap is attributable precisely to what each uses: sampling
baselines collapse each chain to its final answer, whereas band depth exploits
the geometry of the intermediate reasoning. The improvement is largest on
HotpotQA (gBD-v2 77.19 vs.\ 63.78 for the best baseline), where multi-hop
reasoning produces rich intermediate structure for the graph to capture.

\textbf{Graph-level random walks are the most robust signal.} Among our own
methods, the two gBD variants are consistently strongest, with the free variant
gBD-v2 edging out the anchored gBD-v1 in every column where both are measured.
This supports the central design choice of gBD: sampling reference paths from the
full graph topology, rather than only from the observed chains, yields a richer
and more discriminative depth estimate. The exception is 2WikiMultihopQA, where
the vertex-level vBD (69.22) outperforms both gBD variants; we attribute this to
this subset's shorter inference chains, which yield sparse graphs on which
vertex-level centrality is more stable than path-level random walks.
...
We further isolate the two hyperparameters that are specific to the band-depth
construction: the \emph{subset size} used to form each hull, and the
\emph{random-walk length} in gBD. All sweeps below recompute band depth on the
same fixed sampled ensembles as the main results and write their AUROCs to a
separate \texttt{output/ablation/} location, leaving the main results in
\texttt{output/metric/} untouched. The reported numbers are currently pending and
flagged as [TODO: run experiment].
...
Table~\ref{tab:abla-subset} sweeps $J \in \{2,3,4,5\}$ for both vBD and gBD-v2,
holding all else fixed, to determine whether the minimal pairwise band is in fact
the best choice.

% Results to be written to output/ablation/subset_size.json (see output/ablation/README.md)
\begin{table}[t]
  \centering
  \footnotesize
  \setlength{\tabcolsep}{4pt}
  \begin{tabular}{c ccc ccc}
    \toprule
    \multirow{2}{*}{$J$}
      & \multicolumn{3}{c}{\textbf{vBD}} & \multicolumn{3}{c}{\textbf{gBD-v2}} \\
    \cmidrule(lr){2-4}\cmidrule(lr){5-7}
      & GSM8K & SVAMP & HotpotQA & GSM8K & SVAMP & HotpotQA \\
    \midrule
    2 & -- & -- & -- & -- & -- & -- \\
    3 & -- & -- & -- & -- & -- & -- \\
    4 & -- & -- & -- & -- & -- & -- \\
    ...
    \bottomrule
  \end{tabular}
  \caption{Effect of the band subset size $J$ (number of vertices for vBD, paths
  for gBD-v2) on AUROC ($\times 100$), Llama-3.1-8B. Defaults are $J{=}3$ for vBD
  and $J{=}2$ for gBD. [TODO: run experiment].}
  \label{tab:abla-subset}
\end{table}
...
discriminative. Table~\ref{tab:abla-walklen} sweeps $L$ over a range of fixed
values together with the current adaptive default (length scaled to the graph),
for gBD-v2.

% Results to be written to output/ablation/walk_length.json (see output/ablation/README.md)
\begin{table}[t]
  \centering
  \footnotesize
  ...
  \begin{tabular}{l ccc}
    \toprule
    \textbf{Walk length $L$} & GSM8K & SVAMP & HotpotQA \\
    \midrule
    3                  & -- & -- & -- \\
    5                  & -- & -- & -- \\
    7                  & -- & -- & -- \\
    ...
    \bottomrule
  \end{tabular}
  \caption{Effect of the gBD random-walk length $L$ on AUROC ($\times 100$),
  gBD-v2 on Llama-3.1-8B. ``adaptive'' is the current default that scales the
  walk length to the graph. [TODO: run experiment].}
  \label{tab:abla-walklen}
\end{table}

Finally, two dataset-coverage items remain and are likewise flagged as
[TODO: run experiment]: sensitivity to the ensemble size $M$, to quantify how
gracefully each method degrades as fewer chains are sampled; and completing the
Llama-2-13B runs on the remaining datasets and sampling baselines (the ``--''
entries in Table~\ref{tab:main-results}).
